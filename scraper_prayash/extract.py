...
def extractResults(soup):
    ...
    for subject in subjects:
        # In the form: subject_id, subjectCode, subjectCredit, subjectGrade
...
def getExams():
    db.connect()
    # {student_id student_id, semester_id code, sgpa, credits}
    for x in data:
        batch = x.keys()[0]
        print batch
...
# Accumulate Scores
def getScores():
    # {student_id=student_id, subject_id=code, grade, semester_id=semcode, recheckGrade = None}
    db.connect()
    for x in data:
        batch = x.keys()[0]
        print batch
...
# for subject_id in subjects: print "%-34s %s %s %s" % subject_id
# print "SGPA: [%s]" % sgpa
